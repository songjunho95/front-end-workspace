...
$("h4:not(.text)").css({"background-color": "orange", color:"white"});

// 2. Anscestors & Descendants 메서드
// parent : 선택된 요소의 바로 위 상위 요소 선택 
// span 요소의 부모 요소에 글자 색 : red, 테두리 : 2px solid
$("span").parent().css({ color: "red", border: "2px solid"});

// parents (['선택자']) : 선택된 요소의 모든 상위 요소 선택하거나, 매개변수가 있으면 매개변수와 일치하는 상위 요소만 선택 
...
// parentsUntill(['선택자']) : 선택된 요소의 모든 상위 요소를 선택하거나, 매개변수가 있으면 선택된 요소부터 매개 변수와 일치하는 요소까지의 상위 요소 선택
...
$(".wrpa2 h2").siblings().css(stlye3);

// h2 요소의 형제 요소 중 p 요소에 stlye2 적용
$(".wrpa2 h2").siblings("p").css(style2);


// next : 선택된 요소와 같은 레밸 요소 중 다음 한개 요소 선택 
...
$(".wrpa2 h2").nextAll().css("background-color", "pink");

// nextUntill(['선택자']) : 선택된 요소와 같은 레벨 요소 중 다음의 모든 요소를 선택하거나, 매개변수가 있으면 같은 레벨의 다음 요소 중 매개변수와 일치하는 요소까지 선택
// h2 요소 이후부터 p 요소 이전에 모든 형제 요소에 테두리 : 2px dashed
$(".wrap2 h2").nextUntil("p").css("border", "2px dashed");

// prev : 선택된 요소와 같은 레밸 요소 중 이전의 한 개 요소 선택
// h2 요소의 이전 형제 요소에 style5 적용 
$(".wrap2 h2").prev().css(style5);

// preAll : 선택된 요소와 같은 레벨 요소 중 이전의 모든 요소 선택
// h2 요소의 이전에 모든 형제 요소에 배경색 : moccasin
$(".wrap2 h2").prevAll().css("background-color", "moccasin");


// prevUntill(['선택자']) : 선택된 요소와 같은 레벨 요소 중 이전의 모든 요소를 선택하거나, 매개변수가 있으면 같은 레벨의 이전 요소 중 매개변수와 일치하는 요소까지 선택

// h2 요소 이전부터 p 요소 이후까지의 모든 형제 요소에 글자크기 : 2rem
$(".wrap2 h2").prvUntill("p").css("font-size", "2rem");
