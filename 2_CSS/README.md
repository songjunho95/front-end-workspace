...
- CSS 레이아웃 모델로 화면 크기에 따라 레이아웃의 배치나 크기를 조절할 떄 편리하게 사용
...
- 여러 행 정렬
- `flex-wrap: wrap;` 이 설정된 상태에서, 아이템들의 행이 2줄 이상인 경우 수칙축 방향 정렬을 결정하는 속성

```css
.container {
  align-content : stretch / flex-start / flex-end / center / space-around / space-evenly;
}
```
## flex-basis
...
.item{
   ...
   flex : 1 500px;
   /* flex-grow: 1; flex-shrink: 1; flex-basis: auto 500px; */

   flex: 1;
   /* flex-grow: 1; flex-shrink: 1; flex-basis: auto 0%; */
}
```
